...
typedef struct __MIDISpyPort
{
    ...
    MIDIReadBlock readBlock;    // NULL if using readProc
    MIDIReadProc readProc;      // NULL if using readBlock
    void *readProcRefCon;       // Only used if using readProc
...
OSStatus MIDISpyPortCreate(MIDISpyClientRef clientRef, MIDIReadBlock readBlock, MIDISpyPortRef *outSpyPortRefPtr)
{
    ...
    spyPortRef->readProc = NULL;
    spyPortRef->readProcRefCon = NULL;
    CFRetain(readBlock);
...
OSStatus MIDISpyPortCreateWithProc(MIDISpyClientRef clientRef, MIDIReadProc readProc, void *readProcRefCon, MIDISpyPortRef *outSpyPortRefPtr)
{
    MIDISpyPort *spyPortRef;
    ...
    spyPortRef->client = clientRef;
    spyPortRef->readBlock = NULL;
    spyPortRef->readProc = readProc;
    spyPortRef->readProcRefCon = readProcRefCon;
...
static CFDataRef LocalMessagePortCallback(CFMessagePortRef local, SInt32 msgid, CFDataRef data, void *info)
{
    ...
    // and for each, call port->readBlock() or port->readProc().
    ...
    if (endpoint) {
        ...
        if ((connections = GetConnectionsToEndpoint(clientRef, endpoint))) {
            ...
            while (connectionIndex--) {
                ...
                connection = (MIDISpyPortConnection *)CFArrayGetValueAtIndex(connections, connectionIndex);
                if (connection->port->readBlock) {
                    connection->port->readBlock(packetList, connection->refCon);
                } else if (connection->port->readProc) {
                    connection->port->readProc(packetList, connection->port->readProcRefCon, connection->refCon);
                }
            }
        }        
    }
